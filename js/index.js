...
function startGame() {
  ...
  const enemyCharKey =
    selectedCharacter === 'samuraiMack' ? 'kenji' : 'samuraiMack'
  const enemyDataConfig = characterData[enemyCharKey]
  player = new Fighter({
    position: {
      x: 100,
      y: canvas.height - 96 - 150 + playerDataConfig.offset.y
    },
    velocity: { x: 0, y: 0 },
  ...
  enemy = new Fighter({
    position: {
      x: canvas.width - 250,
      y: canvas.height - 96 - 150 + enemyDataConfig.offset.y
    },
    velocity: { x: 0, y: 0 },
    color: 'blue',
  ...
  if (!enemy.dead) {
    if (keys.ArrowLeft.pressed && enemy.lastKey === 'ArrowLeft') {
      ...
      if (distance > 150) {
        ...
        enemy.switchSprite('run')
        enemy.lastKey =
          player.position.x < enemy.position.x ? 'ArrowLeft' : 'ArrowRight'
      } else if (
        distance < 150 &&
        enemy.velocity.y === 0 &&
        now - lastEnemyAction > 500 &&
        Math.random() < 0.05
      ) {
        enemy.attack()
        lastEnemyAction = now
      } else if (
        now - lastEnemyAction > 2000 + Math.random() * 3000 &&
        enemy.velocity.y === 0 &&
        Math.random() < 0.005
      ) {
        enemy.velocity.y = -20
...
document
  .getElementById('samuraiMackPortrait')
  .addEventListener('click', () => selectCharacter('samuraiMack'))
document
  .getElementById('kenjiPortrait')
  .addEventListener('click', () => selectCharacter('kenji'))
